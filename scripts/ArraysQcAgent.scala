...
/**
  * Created by atanas on 9/16/2016.
  */
object SparkMyApp {
  ...
  val prodEnv = System.getProperty("javax.net.ssl.keyStore").contains("broad-gotc-prod")
  ...
  val (myDB, cloudConnectInfo, agentName) =
    if (prodEnv)
      (utils.CognosDB, configToMap(s"metricsCloud.prod"), "analytics.tiger.spark.ArraysQcAgent")
    else
      (utils.DevDB, configToMap(s"metricsCloud.dev"), "analytics.tiger.spark.ArraysQcAgentDev")
...
*/
  ...
  def refreshTable(tableName: String, pk: Seq[String])(df: DataFrame)(session: DBSession) = Try {
    ...
    val count = SQL(s"INSERT INTO $tableName VALUES(${df.schema.map(_ => "?").mkString(",")})").batch(data.map(conv): _*).apply().size
    s"$tableName: $count"
  }
  ...
  val arrayQCReader = Reader((delta: MillisDelta) => {
    ...
    SparkMyApp.tableDF("ARRAYS_QC").where($"MODIFIED_AT" >= d1 && $"MODIFIED_AT" < d2).select("ID","CHIP_WELL_BARCODE","SAMPLE_ALIAS","ANALYSIS_VERSION","IS_LATEST","CHIP_TYPE","AUTOCALL_PF","AUTOCALL_DATE","IMAGING_DATE","IS_ZCALLED","AUTOCALL_GENDER","FP_GENDER","REPORTED_GENDER","GENDER_CONCORDANCE_PF","HET_PCT","HET_HOMVAR_RATIO","CLUSTER_FILE_NAME","P95_GREEN","P95_RED","AUTOCALL_VERSION","ZCALL_VERSION","TOTAL_ASSAYS","TOTAL_SNPS","TOTAL_INDELS","NUM_CALLS","NUM_NO_CALLS","NUM_IN_DB_SNP","NOVEL_SNPS","FILTERED_SNPS","PCT_DBSNP","NUM_SINGLETONS","CALL_RATE","CREATED_AT","MODIFIED_AT","scanner_name","NUM_AUTOCALL_CALLS","AUTOCALL_CALL_RATE")
...
val res =
  SparkMyApp.myDB.apply(MillisDelta.loadFromDb(SparkMyApp.agentName, dbSysdate = _ => new DateTime(SparkMyApp.queryDF("SELECT CURRENT_TIMESTAMP() time").collect()(0).getTimestamp(0).getTime))
    flatMap (prepareEtl(SparkMyApp.agentName, _, SparkMyApp.ArraysQcAgent)())
  )

defaultErrorEmailer(SparkMyApp.agentName)(res)
println(res)
