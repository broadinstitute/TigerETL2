...
USING
    ...
    LEFT JOIN   (SELECT DISTINCT sps.plated_sample_id, sps.pdo_sample_id, sps.pdo, sps.plated_sample_lsid, pdos.on_risk, pdos.risk_types, sps.concentration
	...
	LEFT JOIN bsp.analytics_sample@analytics.gap_prod bsp ON bsp.sample_lsid = s.lsid
    WHERE
        l.creation_time >= ?
        AND l.creation_time < ?
    ...
    JOIN bsp.analytics_sample@analytics.gap_prod s  ON s.sample_barcode = lls.lcset_sample
    WHERE 
...
val agentName = "analytics.tiger.LibrarySampleLcSetEtl"
